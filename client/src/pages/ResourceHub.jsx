...
export default function ResourceHub() {
  ...
  const loadResources = async () => {
    try {
      const res = await axiosInstance.get("/posts/resource");
      setPosts(res.data);
      setLoading(false);
    } catch (err) {
  ...
  const handleCreatePost = async (e) => {
    ...
    try {
      ...
      const res = await axiosInstance.post("/posts/resource", formData, {
  ...
  <div className="flex items-center justify-center w-12 h-12 font-bold text-white bg-indigo-300 rounded-full shadow-md">
    {user?.name?.charAt(0).toUpperCase()}
        ...
        <div className="p-6 mb-8 bg-white border shadow-xl rounded-3xl border-indigo-200/50">
          <h2 className="mb-4 text-xl font-semibold">Share a Resource</h2>

          <form onSubmit={handleCreatePost}>
            <textarea
              className="w-full p-4 border rounded-xl"
              placeholder="Share a resource, link, notes..."
              value={newPost}
